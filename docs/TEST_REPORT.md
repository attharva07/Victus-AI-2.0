...
- **Policy validation:** Policy engine enforces allowlists/denylists, domain boundaries, privacy flags for screenshots/OpenAI, and confirmation for sensitive Gmail actions. Tests cover approval issuance, denylist handling, privacy enforcement (including mixed-domain privacy blocks), and Gmail confirmation requirements. 【F:victus/core/policy.py†L31-L88】【F:victus/core/policy.py†L102-L126】【F:tests/unit/test_policy.py†L1-L200】
- **Executor safeguards:** Execution fails without approvals, missing signatures, unregistered plugins, or unapproved step IDs; it delegates to plugins only after argument validation. Unit tests confirm these safeguards and reject tampered approvals/signatures. 【F:victus/core/executor.py†L12-L35】【F:tests/unit/test_executor.py†L1-L120】
